...
#if 0
int appendToAnnotFile(vector<string> &storeSign){
std::ofstream myfile;
myfile.open("annot/UserFunc.annot",ios::out | ios::trunc);
if(!myfile)
{
...
vector<string>::iterator it=storeSign.begin();
while(it!=storeSign.end())
myfile<<*(it++)<<endl;
myfile.close();
return 0;
}
...
void funcAnnotProcessing(SgProject* project){
typedef Rose_STL_Container<SgNode*> RSTL;

SgFilePtrList & ptr_list = project->get_fileList();
for(SgFilePtrList::iterator it = ptr_list.begin(); it!=ptr_list.end();++it)
{
vector<string> storeFuncSign;
SgSourceFile *sfile=isSgSourceFile(*it);
...
for(iter=funcExpList.begin();iter!=funcExpList.end();++iter)
{
  ...
  if(fexp->getAssociatedFunctionDeclaration()!=NULL)
  {
  ...
  if(forLoopList.size()>0)
    ...
    else
    {
        ...
        //cout<<funcSign<<endl;
        storeFuncSign.push_back(funcSign);       
    }
  }
}
}
appendToAnnotFile(storeFuncSign);
}
}
#endif

void changeNameOfFile(SgNode* node)
{
 SgSourceFile* sfile;
 SgFile* cur_file;
 string orig_name ;
 ...
 int i;
  ...
  sfile = isSgSourceFile(node);
  cur_file=sfile;
  ROSE_ASSERT(cur_file);
  orig_name = cur_file->get_file_info()->get_filenameString();
  file_suffix = StringUtility::fileNameSuffix(orig_name);
  if (CommandlineProcessing::isCFileNameSuffix(file_suffix))
  {
   orig_name = StringUtility::stripPathFromFileName(orig_name);
   naked_name = StringUtility::stripFileSuffixFromFileName(orig_name);
   cur_file->set_unparse_output_filename(naked_name+"_omp45.c");
  }
...
int main (int argc, char *argv[])
{
  ...
  {                             
        ...
        iter++)
    {
	  ...
	  Rose_STL_Container<SgNode*> nestForList=NodeQuery::querySubTree(current_loop,V_SgForStatement);
