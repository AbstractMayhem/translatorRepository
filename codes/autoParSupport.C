...
namespace AutoParallelization
{
  ...
  std::map< std::string,std::pair< int , std::vector<int> > > storeDynArr;
  ...
  void CollectVisibleVariables(SgNode* loop, std::vector<SgInitializedName*>&
  ...
  void AutoScoping(SgNode *sg_node,OmpSupport::OmpAttribute* attribute,LoopTreeDepGraph* depgraph)
  {
    ...
    if (enable_debug)
    {
      cout<<"Debug after CollectVisibleVariables ():"<<endl;
...
//cout<<for_stmt->unparseToString()<<endl;
{
...
omp_attr->addExpression(OmpSupport::e_collapse,"", SageBuilder::buildIntVal(counter));
}
}
#endif


void Offload(SgNode *loop,OmpSupport::OmpAttribute *omp_tparallel_attr){

SgForStatement *forStmt=isSgForStatement(loop);
ROSE_ASSERT(forStmt!=NULL);

OmpSupport::OmpAttribute *omp_tdata_attr = OmpSupport::buildOmpAttribute(e_target_data,forStmt,false);
//OmpSupport::OmpAttribute *omp_tparallel_attr = OmpSupport::buildOmpAttribute(e_parallel_for,forStmt,false);
ROSE_ASSERT(omp_tdata_attr && omp_tparallel_attr);

std::vector<SgInitializedName*> allVars,invariantVars;
...
findReductionVars(forStmt,reductionVars,omp_tparallel_attr);
//recognizeCollapse(forStmt,omp_tparallel_attr);
...
//Add Clause
omp_tdata_attr->addClause(OmpSupport::e_map);
//Add To Variables to attribute
if(toVars.size()!=0)
{
omp_tdata_attr->setMapVariant(OmpSupport::e_map_tofrom);
mapVarsToOmpAttribute(omp_tdata_attr,OmpSupport::e_map_tofrom,toVars);
}
if(fromVars.size()!=0)
{
//Add From Variables to attribute
omp_tdata_attr->setMapVariant(OmpSupport::e_map_tofrom);
mapVarsToOmpAttribute(omp_tdata_attr,OmpSupport::e_map_tofrom,fromVars);
}
if(toFromVars.size()!=0)
{
//Add ToFrom Variables to attribute
omp_tdata_attr->setMapVariant(OmpSupport::e_map_tofrom);
mapVarsToOmpAttribute(omp_tdata_attr,OmpSupport::e_map_tofrom,toFromVars);
}
...
OmpSupport::addOmpAttribute(omp_tdata_attr,forStmt);
//Generate Attribute
OmpSupport::generatePragmaFromOmpAttribute(forStmt);
OmpSupport::removeOmpAttribute(omp_tdata_attr,forStmt);//This statement for removing openmp target data attribute and then adding openmp target teams distribute attribute
}
...
{
//recognizeCollapse(forStmt,omp_tparallel_attr);
//omp_tparallel_attr->setOmpDirectiveType(OmpSupport::e_parallel_for);
OmpSupport::addOmpAttribute(omp_tparallel_attr,forStmt);
OmpSupport::generatePragmaFromOmpAttribute(forStmt);
}
